...
public abstract class BaseCreatePostPresenter<V extends BaseCreatePostView> extends PickImagePresenter<V> implements OnPostCreatedListener {
    ...
    protected abstract void savePost(final String title, final String description, final String alamat,
    ...
    protected void attemptCreatePost(Uri imageUri) {
        ...
        ifViewAttached(view -> {
            ...
            String status = view.getStatusText().trim();
            Double longitude = view.getLongitudeText().doubleValue();
            Double latitude = view.getLatitudeText().doubleValue();
            ...
            if (TextUtils.isEmpty(alamat)) {
                view.setAlamatError(context.getString(R.string.warning_empty_alamat));
                cancel = true;
            }
            ...
            if (!cancel) {
                creatingPost = true;
                view.hideKeyboard();
                savePost(title, description,alamat,longitude,latitude,status);
            }
        });
    }
