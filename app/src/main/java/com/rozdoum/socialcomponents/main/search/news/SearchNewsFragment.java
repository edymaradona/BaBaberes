...
import com.rozdoum.socialcomponents.adapters.SearchNewsAdapter;
...
import com.rozdoum.socialcomponents.main.search.posts.SearchPostsPresenter;
...
public class SearchNewsFragment extends BaseFragment<SearchNewsView, SearchNewsPresenter>
        implements SearchNewsView, Searchable {
    ...
    public SearchNewsPresenter createPresenter() {
        if (presenter == null) {
            return new SearchNewsPresenter(getContext());
        }
        return presenter;
    }
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container,
        ...
        emptyListMessageTextView.setText(getResources().getString(R.string.empty_news_search_message));
                ...
                case PostDetailsActivity.UPDATE_POST_REQUEST:
                    if (data != null) {
                        ...
                        if (postStatus.equals(PostStatus.REMOVED)) {
                            newsAdapter.removeSelectedNews();

                        } else if (postStatus.equals(PostStatus.UPDATED)) {
                            newsAdapter.updateSelectedNews();
                        }
                    }
                    break;
            }
    ...
    private void initRecyclerView() {
        newsAdapter = new SearchNewsAdapter((BaseActivity) getActivity());
        newsAdapter.setCallBack(new SearchNewsAdapter.CallBack() {
            @Override
            public void onItemClick(News news, View view) {
                NewsManager.getInstance(getActivity().getApplicationContext()).isNewsExistSingleValue(news.getId(), new OnObjectExistListener<News>() {
                    @Override
                    public void onDataChanged(boolean exist) {
                        if (exist) {
                            //openPostDetailsActivity(post, view);
                        } else {
                            showSnackBar(R.string.error_news_was_removed);
                        }
                    }
                });
            }
    ...
    @Override
    public void onSearchResultsReady(List<News> news) {
        hideLocalProgress();
